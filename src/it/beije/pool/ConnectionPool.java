...
public class ConnectionPool {
	...
	private static MyConnection[] connections = null; // tutte le connsioni puntano a null(0-9)
	//alla conn. 0-> 0 false null.0
		...
		for (MyConnection c : connections) {
			if (!c.isAvailable()) {
				if (c.getTimestamp() == null ||
					c.getTimestamp().plus(expiredTime).isBefore(LocalDateTime.now())// tot di tempo(expiredTime))
					) {
					c.close();
				}
			}
